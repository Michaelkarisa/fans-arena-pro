...
exports.getStreamData = functions.runWith({
  ...
  let MUX_ACCESS_TOKEN:string="";
  let  MUX_SECRET_KEY :string='';
  ...
  try {
    ...
    for (const doc of snapshots.docs) {
      ...
      if (d && d.lat != undefined && d.long != undefined && d["radius(km)"] != undefined) {
        const isWithinDistance = haversineDistance(
          data.lat,
          data.long,
          d.lat,
          d.long,
      ...
      try {
        ...
        if (!viewsSnapshot.empty) {
          ...
          viewsSnapshot.docs.forEach((viewDoc) => {
            ...
            const timestamp = view.timestamp?.toDate(); // Ensure timestamp is a Firestore Timestamp
            if (!timestamp) return;
        ...
        const adAnalytics = {
          ...data, // Include the ad data
          mostViewsHour: `${mostViewsIntervalIndex * 2}:00 to ${(mostViewsIntervalIndex * 2) + 2}:00`, // Show the 2-hour interval with the most views
          mostViewsDay: getDayName(mostViewsDay), // Convert day index to name (Sunday, Monday, ...)
          viewsByTimeOfDay,
          viewsByDayOfWeek,
        };
        ...
        const adAnalytics = {
          ...data, // Include the ad data
          mostViewsHour: "No Views", // Default value for no views
          mostViewsDay: "No Views", // Default to Sunday
          viewsByTimeOfDay: new Array(12).fill(0),
          viewsByDayOfWeek: new Array(7).fill(0),
        };
...
exports.getVideoData = functions.runWith({
  ...
  try {
    ...
    const currentUserUid = queryParams["authorId"] as string;
...
function isWithin24Hours(firestoreTimestamp: admin.firestore.Timestamp): boolean {
...
exports.getPostsForFollowedUsers = functions.runWith({
  ...
  try {
    ...
    const collection = queryParams["collection"] as string;
    if (!currentUserUid) {
      response.status(400).json({ error: "User ID is required" });
      return;
    }
    ...
    const collectUids = (snapshot: admin.firestore.QuerySnapshot, key: string) => {
      snapshot.forEach((doc) => {
        const followingData = doc.data()[key] as { userId: string }[];
        followingData.forEach((item) => {
          if (item.userId) followingUids.push(item.userId);
        });
      });
    };

    // Helper function to collect UIDs from clubsteam snapshots
    const collectUidsFromClubsteam = (snapshot: admin.firestore.QuerySnapshot, key: string) => {
      snapshot.forEach((doc) => {
        ...
        if (clubsTeamTable[1]) {
          ...
          clubsteam.forEach((clubItem: { [key: string]: string }) => {
            if (clubItem[fieldName]) followingUids.push(clubItem[fieldName]);
          });
        }
      });
    };
    ...
    fromprofeclubSnapshot.forEach((doc) => {
      if (doc.id) followingUids.push(doc.id);
    });
    ...
    const stories = ([] as any[]).concat(...storiesArray);
    ...
    let enrichedVideos: any[] = []; // Initialize variables outside the condition
    let allUsers: any[] = [];
    if(collection=="Fan"){
...
exports.getPostById = functions.runWith({
  ...
  try {
    const queryParams = convertParsedQs(request.query);
    const postId = queryParams["postId"] as string;
    ...
    const captionUrl = await getImageAspectRatios(postData?.captionUrl);
...
exports.getSavedPosts = functions.runWith({
  ...
  try {
    ...
    savedPostsSnapshot.forEach((doc) => {
      const savedPosts = doc.data().posts as { postId: string, timestamp: FirebaseFirestore.Timestamp }[];
...
exports.getSavedFansTv = functions.runWith({
  ...
  try {
    ...
    savedPostsSnapshot.forEach((doc) => {
      const savedPosts = doc.data().FansTv as { postId: string, timestamp: FirebaseFirestore.Timestamp }[];
     savedPosts.forEach((item) => {
        if (item.postId) {
          postIds.push(item.postId);
        }
...
async function fetchUserData(authorId: string): Promise<{profileImage: string, username: string, collectionName: string, planETime: admin.firestore.Timestamp, plan: string, location: string, motto: string, userId: string, docRef: DocumentReference, country: string, token: string, timestamp: any,email:string}> {
  let username: string = '';
  let imageurl: string = '';
  let collectionName = '';
  let location: string = '';
  ...
  let plan: string = "";
  let planETime: admin.firestore.Timestamp = admin.firestore.Timestamp.now();  // Default value
  let email:string="";
  if (authorId) {
    ...
    if (clubSnapshot.exists) {
      ...
      if (clubData?.plan !== undefined) {
        plan = clubData?.plan;
        planETime = clubData?.planETime || admin.firestore.Timestamp.now();
      }
    } else {
      ...
      if (professionalSnapshot.exists) {
        ...
        if (professionalData?.plan !== undefined) {
          plan = professionalData?.plan;
          planETime = professionalData?.planETime || admin.firestore.Timestamp.now();
        }
      } else {
  ...
  .onCreate(async (snap) => {
    try {
      ...
      const promises = useruids.map(async (followerId) => {
        const userData= await fetchUserData(followerId as string);
        if (userData !== undefined) {
...
.onCreate(async (snap) => {
    try {
      ...
      const updatedStories = await Promise.all(
        stories.map(async (story: { thumbnail: undefined; url: any; storyId: any; }) => {
          ...
          if (!story.thumbnail||story.thumbnail==undefined) {
            ...
            try {
              ...
              return { ...story, thumbnail: newThumbnail }; // Update the story with the new thumbnail
            } catch (error) {
    ...
    .onCreate(async (snap) => {
        ...
        if(thumbnails){
          const updatedHighlights = await Promise.all(
            highlights.map(async (highlight: { urls: any; highlightId: any; }) => {
              ...
              return { ...highlight, urls: updatedUrls };
            })
          );
...
if(token){
    ...
    const sentFrom = new Sender("dripakenya@gmail.com", "Dripa");
...
.onRequest(async (request, response) => {
  try {
    ...
    const messag: string | undefined = queryParams["message"] as string;
...
.onRequest(async (request, response) => {
  try {
    ...
    const leagueId: string | undefined = data.leagueId as string;
    const matchId: string | undefined = data.matchId as string;
    const match1Id: string | undefined = data.match1Id as string;
    const club1: string | undefined = data.club1 as string;
    const club2: string | undefined = data.club2 as string;
    const leaguematchId: string | undefined = data.leaguematchId as string;
  ...
  const getclubsF = async (club:string) => {
      ...
      // Fetch user data for current user
      const sendClubNotification = async (userData:any,matchId:string) => {
        if (userData !== undefined) {
          const token =userData.token;
          if (token!==undefined) {
            const message = {
...
function checkAndRespond(input: string):string{
  const responses: { phrase: string; response: string }[] = [
...
exports.getreplydata = functions.runWith({
  ...
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      repliesData.push(...data.filter((reply: { commentId: any; }) => reply.commentId === commentId));
    });
...
exports.getanydata = functions
  ...
  .https.onRequest(async (request, response) => {
    try {
      const docId = request.body.docId as string;
      const collectionName = request.body.collection as string;
      const subcollectionName = request.body.subcollection as string;
      const length = request.body.length as number;
      ...
      if (subcollectionName === "notifications") {
        ...
        profesSnapshot.forEach((doc) => {
          const dataArray = doc.data().notifications as Notification[];
        ...
        // Handle other subcollections dynamically
        interface SubcollectionItem {
          userId: string;
          timestamp: FirebaseFirestore.Timestamp;
        }

        const subcollectionData: SubcollectionItem[] = [];

        profesSnapshot.forEach((doc) => {
          const dataArray = doc.data()[subcollectionName] as SubcollectionItem[];
          if (Array.isArray(dataArray)) {
...
functions.runWith({
  ...
  try {
    const docId = request.body.docId as string;
    const year = request.body.year as string;
    const length = request.body.length as number;
    if (!docId || !year) {
response.status(400).json({
...
functions.runWith({
  ...
  try {
    const docId = request.body.docId as string;
    const year = request.body.year as string;
    const commentId=request.body.commentId as string;
    const length = request.body.length as number;
...
const slicedReplies = replies1
      ...
      try {
       ...
       // const streamId = data.streamId; 
        ...
        if (converterId !== undefined) {
          ...
          try {
            const response = await axios.delete(agoraUrl, {
              auth: {
            ...
            if (response.status === 200 || response.status === 204) {
              ...
              await agoraapis.ref.update({
                isLive:false,
                stoptime:admin.firestore.Timestamp.now()}); // Check for 200 or 204 status codes
  ...
  //let imageUrl:string='';
   ...
   const userData= await fetchUserData(data.userId);
        ...
        if (collection === "Clubs") {
          ...
          if (!clubSnapshot.empty) {
            for (const doc of clubSnapshot.docs) {
              ...
              const clubsteam: Record<string, any>[] = doc.data()["clubsteam"] || [];
              const clubsTeamTable: Record<string, any>[] = doc.data()["clubsTeamTable"] || [];
          ...
          if (leagueSnapshot.exists) {
            const leagueData = leagueSnapshot.data();
            const leagueTable: Record<string, any>[] = leagueData?.[data.table] || [];
            ...
            if (!clubSnapshot.empty) {
              for (const doc of clubSnapshot.docs) {
                docIds.push(doc.id);
                const clubs: Record<string, any>[] = doc.data()[data.data] || [];
    ...
    async function enrichTeamData(clubsteam: Record<string, any>[], clubsTeamTable: Record<string, any>[]): Promise<Record<string, any>[]> {
      const enrichedTable: Record<string, any>[] = [];
    ...
    async function enrichTeamData1(scorers: Record<string, any>[], scorersTable: Record<string, any>[]): Promise<Record<string, any>[]> {
      const enrichedTable: Record<string, any>[] = [];
 ...
 * Runs for multiple collections and subcollections based on the provided list.
 */
    async function optimizeAllCollections(): Promise<void> {
    ...
    async function optimizeSubcollectionArray(
      ...
      try {
        ...
        for (const doc of docs) {
          const data = doc.data();
          const array: any[] = data[fieldName] || [];
          allItems.push(...array);
...
async function generateAndUploadThumbnail(videoUrl: string):Promise<string> {
  try {
    const agoraapis = await admin.firestore().collection("APIS").doc("api").get();
    const data = agoraapis.data();
    ...
    const outputPath = path.join(tmpDir, `thumbnail-${Date.now()}.jpg`);
    ...
    const secureUrl = await new Promise((resolve, reject) => {
      ffmpeg(videoUrl)
        ...
        .on("stderr", (stderrLine: any) => console.log(`FFmpeg STDERR: ${stderrLine}`))
        .on("end", async () => {
    ...
    while (allDataList.length > 0) {
      ...
      if (item) {
        if(item.collection=="Leagues"){
          ...
          let querySnapshot: admin.firestore.QuerySnapshot<admin.firestore.DocumentData>;
          let isNewDocument = true;
         ...
         if (querySnapshot.docs.length>0) {
           ...
           for (const doc of querySnapshot.docs) {
            ...
            const matchingViewIndex = collection === "Story"
              ? views.findIndex((view: any) => view.userId === data1.userId && view.storyId === data1.storyId)
              : views.findIndex((view: any) => view.userId === data1.userId);
            ...
            if (matchingViewIndex !== -1) {
              ...
              const updatedTimestamp =
                matchingView.timestamp instanceof admin.firestore.Timestamp &&
                matchingView.timestamp.toMillis() < data1.timestamp.toMillis()
                  ? matchingView.timestamp
                  : data1.timestamp;
          
              views[matchingViewIndex] = {
                ...matchingView,
                watchhours: updatedWatchhours,
          ...
          await Promise.all(updatePromises);
           ...
           if (!isUpdated) {
             ...
             if (latestDoc && latestDoc.data()) {
               const views = latestDoc.data()?.views || [];
...
if (item.subcollection === "likes") {
  ...
  const userIdExists = notifications.some((notification: any) => notification.userId === data.userId);
  if (!userIdExists) {
    if (isNewDocument) {
        ...
        if(docData){
         const updateData: { [key: string]: any } = { 
          ...
          if (!createdAt) { 
            ...
            await latestDoc.ref.update(updateData); } 
          } else { console.log('User ID already exists in likes, not adding entry.'); 
...
exports.matchData = functions.runWith({
  ...
  try {
    ...
    const matchTickets: TicketData[] = [];
    if (!matchTicketsSnapshot.empty) {
      matchTicketsSnapshot.forEach((doc) => {
        const ticketsList = doc.data().likes as TicketData[] || [];
    ...
    if (!matchDonationsSnapshot.empty) {
      matchDonationsSnapshot.forEach((doc) => {
        const donationsList = doc.data().donations as DonationData[] || [];
    ...
    const likesPerInterval: number[] = Array(intervalsInDay).fill(0);
    const ticketsPerInterval: number[] = Array(intervalsInDay).fill(0);
    const viewsPerInterval: number[] = Array(intervalsInDay).fill(0);
    const adViewsPerInterval: number[] = Array(intervalsInDay).fill(0);
    const watchhoursPerInterval: number[] = Array(intervalsInDay).fill(0);
    const adWatchhoursPerInterval: number[] = Array(intervalsInDay).fill(0);
    const donationsPerInterval: number[] = Array(intervalsInDay).fill(0);
    const amountPerInterval: number[] = Array(intervalsInDay).fill(0);

    // Helper function to get the two-hour interval of the day from a timestamp
    const getIntervalOfDay = (timestamp: Date) => Math.floor(timestamp.getHours() / 2);
    ...
    const likesDatapoints: LikesDataPoint[] = likesPerInterval.map((likes, interval) => ({
      hour: interval * 2,
      likes,
    }));

    const ticketsDatapoints: TicketsDataPoint[] = ticketsPerInterval.map((tickets, interval) => ({
      hour: interval * 2,
      tickets,
    }));

    const viewsDatapoints: ViewsDataPoint[] = viewsPerInterval.map((views, interval) => ({
      hour: interval * 2,
      views,
      watchhours: watchhoursPerInterval[interval],
    }));
    const adViewsDatapoints: ViewsDataPoint[] = adViewsPerInterval.map((views, interval) => ({
      hour: interval * 2,
      views,
      watchhours: adWatchhoursPerInterval[interval],
    }));
    const donationsDatapoints: DonationsDataPoint[] = donationsPerInterval.map((donations, interval) => ({
      hour: interval * 2,
...
exports.userData = functions.runWith({
  ...
  try {
    ...
    const convertToListOfMaps = (data: Record<string, number>) => {
      return Object.entries(data).map(([key, value]) => ({
        date: key,  // key as date
...
async function reminderUser(): Promise<void> {
  const now = new Date();
  let day = now.getDate();
  ...
  try {
    ...
    const users = await Promise.all(
      querySnapshot.docs.map(async (doc: DocumentSnapshot) => {
        const data = doc.data();
        return {
          email: data?.email,
          token: data?.token,
  ...
  try {
    ...
    const users1 = await Promise.all(
      querySnapshot1.docs.map(async (doc: DocumentSnapshot) => {
        const data = doc.data();
        return {
          email: data?.email,
          token: data?.token,
...
async function runIfFirstOfMonth(): Promise<void> {
  const today = new Date();
  if (today.getDate() !== 1) {
  ...
  const monthOffset = monthAsNumber ?? 0;
  const now = new Date();
    ...
    for (const collection of ["Matches", "Events"]) {
      ...
      for (const matchDoc of matchesSnapshot.docs) {
        ...
        matchViewsSnapshot.forEach((doc) => {
          ...
          totalWatchhours += viewsList.reduce((sum: any, view: { watchhours: any; }) => sum + (view.watchhours || 0), 0);
        });
        matchadViewsSnapshot.forEach((doc) => {
          const viewsList = doc.data().views || [];
          totaladViews += viewsList.length;
          totaladWatchhours += viewsList.reduce((sum: any, view: { watchhours: any; }) => sum + (view.watchhours || 0), 0);
        });
        matchDonationsSnapshot.forEach((doc) => {
          const donationList = doc.data().donations || [];
          totalDonations += donationList.length;
          amount += donationList.reduce((sum: any, donation: { amount: any; }) => sum + (donation.amount || 0), 0);
        });
        matchTicketsSnapshot.forEach((doc) => {
...
exports.allMatchData = functions.runWith({
  ...
  try {
    ...
    const collection: string | undefined = queryParams['collection'] as string;
    const monthString = queryParams['month'] as string;
    const monthAsNumber: number | undefined = monthString && !isNaN(parseInt(monthString, 10)) ? parseInt(monthString, 10) : undefined;
    ...
    const matchTicketsSnapshot = await admin.firestore()
      ...
      matchDonationsSnapshot.forEach((doc) => {
        const donationList = doc.data().donations as DonationData[]||[];
        totalDonations += donationList.length;
        amount += donationList.reduce((sum, donation) => sum + (donation.amount || 0), 0);
      });

      matchTicketsSnapshot.forEach((doc) => {
        const ticketsList = doc.data().tickets as LikeData[]||[];
        totalTickets += ticketsList.length;
      });
      const day = starttime.getDate();
...
exports.handleTransaction = functions.runWith({
  ...
  try {
    ...
    try {
      referenceList = reference as string[];
    ...
    if (userId) {
      ...
      if (userData) {
        ...
        await sendEmail2(userEmail, username, collectionName, purchaseDetails, amount as string, currency as string);
      } else {
...
exports.updatePlan = functions.runWith({
  ...
  try {
    ...
    let planETime: admin.firestore.Timestamp;
...
exports.postData = functions.runWith({
  ...
  try {
    ...
    const daily: Array<{ date: string, likes: number, views: number, watchhours: number }> = [];
    const weekly: Array<{ date: string, likes: number, views: number, watchhours: number }> = [];
    const year: Array<{ date: string, likes: number, views: number, watchhours: number }> = [];
    ...
    matchLikes.forEach(like => {
      const likeTimestamp = like.timestamp.toDate();
      if (likeTimestamp.getFullYear() !== currentYear) return; // Skip if not current year
    ...
    matchViews.forEach(view => {
      const viewTimestamp = view.timestamp.toDate();
      if (viewTimestamp.getFullYear() !== currentYear) return; // Skip if not current year
...
async function fetchCurrentAddress(latitude: number, longitude: number): Promise<AddressResult> {
  console.log("Fetching API key from Firestore...");
  const doc = await admin.firestore().collection("APIS").doc("api").get();
  const mapsApi = doc.data()?.mapsApi;
  ...
  try {
    ...
    if (geocodeResponse.status === 200) {
      ...
      if (data.results && data.results.length > 0) {
        ...
        const addressDetails = addressComponents.map((component: any) => component.long_name);
        ...
        const placesResponse = await axios.get(
          `${placesApiUrl}?location=${latitude},${longitude}&radius=2000&key=${mapsApi}`
        );
        ...
        if (placesResponse.status === 200) {
          ...
          if (placesData.results && placesData.results.length > 0) {
            nearbyPlaces = placesData.results.map((place: any) => place.name);
          } else {
